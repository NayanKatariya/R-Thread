...
const SignUp = () => {
  ...
  useEffect(() => {
    let timerInterval;
    if (show && !resendEnabled && resendTimer > 0) {
  ...
  }, [show, resendEnabled, resendTimer]);
  ...
  const submitHandler = (e) => {
    ...
    if (!show && email && password) {
      sendVerificationCode()
    } else if (show && email && password && verificationCode) {
      verifyCode()
    }
  }
  ...
  const sendVerificationCode = async () => {
    const data = await callApi({
      method: "POST",
      data: { email, password },
      url: `${import.meta.env.VITE_API_BASE_URL}/auth/send-verification-code`,
    });
    if (data?.isSuccess) {
  ...
  const verifyCode = async () => {
    const data = await callApi({
      ...
      url: `${import.meta.env.VITE_API_BASE_URL}/auth/verify-code`,
    });
    if (data?.isSuccess) {
  ...
  const resendCode = async () => {
    const data = await callApi({
      method: "POST",
      data: { email },
      url: `${import.meta.env.VITE_API_BASE_URL}/auth/resend-code`,
    });
    if (data?.isSuccess) {
    ...
    <div className="bg-white rounded-lg ">
      <div className="min-h-screen flex flex-col p-6">
        ...
        <header className="flex justify-between items-center w-full">
          ...
          <Link to="/signin" className="text-sm font-medium  bg-accent text-accent-foreground px-4 py-2 rounded-md">
            Login
          </Link>
        </header>
        ...
        <main className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
          <div className="w-full space-y-6">
            <div className="  font-semibold">
              ...
              <p className="text-2xl text-[#b7b7b7] ">Create your thread account</p>
            </div>

            <div className="space-y-4">
              <Button variant="outline" className="w-full text-sm items-center justify-center gap-2 size:6 py-4" onClick={() => handleLoginWithGoogle(`${import.meta.env.VITE_WEB_URL}/signup`)}>
                <FcGoogle size={50} />
              ...
              <div className="space-y-2 border-t mt-3 pt-8 ">
                ...
                {show && <>
                  ...
                  <Input id="code" type="string" placeholder="Enter your code..." className="mt-1  py-5" onChange={(e) => {
              ...
              {show && (
                <p className="text-base text-center text-[#007AFF] w-[75%] mt-2 mx-auto cursor-pointer">
                  {resendEnabled ? (
                    <span onClick={resendCode}>Resend verification code</span>
                  ) : (
